...
class MyDB {
    ...
    function open() {
        ...
        if ($this->pConnect == 'true') {
            $this->link = new mysqli("p:" . $this->serverAddress, $this->username, $this->password, $this->database);
        } else {
            $this->link = new mysqli($this->serverAddress, $this->username, $this->password, $this->database);
        }

        if (!$this->link or $this->link->connect_errno) {
            $this->errorOutput("Connect to the database failed - " . $this->link->connect_errno);
        }
    ...
    function query($query) {
        ...
        $results = $this->link->query($query) or $this->errorOutput($query);

        return $results;
    }

    /**
     * This function takes a multiple query statement and returns a result set.
     * If there is an error it will execute die output the error.
     *
     * returns
     */
    function multipleQueries($queries) {
        //This is only here to debug and track sql calls
        if (MY_DB_DEBUGGER == true) {
            array_push($this->DEBUG_ARRAY, $queries);
        }

        $results = mysqli_multi_query($this->link, $queries);
    ...
    function queryData($query = "") {
        ...
        if ($resultSet) {
            while ($d = $resultSet->fetch_array(MYSQLI_ASSOC)) {
                $data[] = $d;
            }
        }
    ...
    function cleanParameter($param) {
        return $this->link->real_escape_string($_REQUEST[$param]);
    }
    ...
    function errorOutput($query) {
        die('<div style="margin:10px;border:1px solid #aaa"><font color="#000000"><b>' .
                $this->link->errno . ' - ' . $this->link->error . '<br><br>' . $query .
    ...
    function autocommit($commit = false) {
        $this->link->autocommit($commit);
    }
