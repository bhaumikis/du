...
<script type="text/javascript">
    ...
    $(document).ready( function(){
        ...
        $("#forgotpassword").validate({
            ...
            messages: {
                security_question_id:{required:"<?php echo _l('Error_Select_Question', 'forgot_password'); ?>"},
                security_answer:{required:"<?php echo _l('Error_Enter_Answer', 'forgot_password'); ?>"},
                email:{required:"<?php echo _l('Error_Enter_Email', 'forgot_password'); ?>",email:"<?php echo _l('Error_Enter_Valid_Email', 'forgot_password'); ?>"}
            }
        });
        $(".btnSubmit").click(function() {
            getMSDrowpdownValidate('forgotpassword','security_question_id','<?php echo _l('Error_Select_Question', 'forgot_password'); ?>');
        });
    });
</script>
<div class="f-pass-form">
    <div class="custom-form"> <span class="sq"><?php echo _l('Text_Select_Security_Question', 'forgot_password'); ?></span>
        <form name="forgotpassword" id="forgotpassword" method="post">
            <div class="sec">
                <select class="form-control" name="security_question_id" id="security_question_id">
                    <option value=""><?php echo _l('Label_Select_Security_Question', 'forgot_password'); ?></option>
                    <?php
                    if (isset($view->securityquestions)) {
                        foreach ($view->securityquestions as $questions) {
                            ?>
                            <option class="option-select-css" value="<?php echo $questions['security_question_id']; ?>" <?php echo ($_POST['security_question_id'] == $questions['security_question_id']) ? 'selected="selected"' : ""; ?>><?php echo $questions['question']; ?></option>
                        <?php }
                    } ?>
                </select>
            </div>
            <input type="password" placeholder="<?php echo _l('Label_Enter_Answer', 'forgot_password'); ?>" name="security_answer" id="security_answer" value=""/>
            <span class="f-pass-text"><?php echo _l('Label_Enter_Email', 'forgot_password'); ?></span>
            <input type="text" placeholder="<?php echo _l('Label_Enter_Email', 'forgot_password'); ?>" name="email" id="email" value="<?php echo ($_POST['email'] != "") ? $_POST['email'] : ""; ?>"/>
            
            <span class="f-pass-text"><?php echo recaptcha_get_html($view->captcha_public_key, $error, ((isset($_SERVER["HTTPS"]) and strtolower($_SERVER["HTTPS"]) == "on") ? true : false));?></span>
            <input type="submit" name="submit" value="<?php echo _l('Button_Send', 'common'); ?>" class="f-pass-btn btnSubmit">
